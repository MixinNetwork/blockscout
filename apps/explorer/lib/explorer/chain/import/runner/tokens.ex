    ...
    from(
      ...
      update: [
        set: [
          ...
          asset_id: fragment("EXCLUDED.asset_id")
        ]
      ],
      where:
        fragment(
          "(EXCLUDED.name, EXCLUDED.symbol, EXCLUDED.total_supply, EXCLUDED.decimals, EXCLUDED.type, EXCLUDED.cataloged, EXCLUDED.skip_metadata, EXCLUDED.asset_id) IS DISTINCT FROM (?, ?, ?, ?, ?, ?, ?, ?)",
          token.name,
          token.symbol,
          token.total_supply,
          token.decimals,
